...
class PATZJetsExpress : public edm::EDAnalyzer {
	...
	private:
		...
		struct PARTICLE {
			...
			// --- Trigger Matching
			int TriMatchF1Path, TriMatchF2Path, TriMatchF3muPath, TriMatchF3elePath, TriMatchF4Path, TriMatchF5Path,
			    TriMatchF6Path;
		};
		...
		double mMinJetPt,mMaxJetEta,mMinLepPt,mMaxLepEta,mMaxCombRelIso03,mMaxCombRelIso04,mJetLepIsoR,mJetPhoIsoR, 
		...
		// ---- jet kinematics --------------------------------------------
		vector<float> *jetPt_,*jetPtRES_,*jetPtRESup_,*jetPtRESdown_,*jetEta_,*jetPhi_,*jetE_,
		              *jetPtGEN_,*jetPtRESGEN_,*jetPtRESupGEN_,*jetPtRESdownGEN_,*jetEtaGEN_,*jetPhiGEN_,*jetEGEN_;
		...
		vector<float> *jetBeta_,*jetBtag_,*jetTagInfoNVtx_,*jetTagInfoNTracks_,*jetTagInfoVtxMass_,
		              *jetArea_,*jetJEC_,*jetUNC_,*jetQGL_,*jetRMS_;
		...
		std::vector<float> getESHits(double X, double Y, double Z, std::map<DetId, EcalRecHit> rechits_map, 
		                             const CaloGeometry& geometry, CaloSubdetectorTopology *topology_p, int row=0);
						...
						case 1:
							...
							for(j_gen=gen->begin();j_gen !=gen->end();j_gen++){
								if(j_gen->status() == 1 && abs(j_gen->pdgId()) ==22){
									...
									if(DR<mDressedRadius){ 
										lepP4GEN+=phoP4GEN; 
									}
								}//if: st=1, pdgid=22
							}//for: gen particle (2nd loop)
							break;
						case 2: case 0: default: break;
					}
			...
			for(reco::PhotonCollection::const_iterator it = photons_->begin();it != photons_->end(); it++) {
				...
				if(ecalRecHitSumEtConeDR03 < 6.0 + 0.012*gammaPt) // requirement of _IsoVL_ type photon triggers 
					if(hcalTowerSumEtConeDR03  < 4.0 + 0.005*gammaPt)
						if(trkSumPtHollowConeDR03  < 4.0 + 0.002*gammaPt)
							isTriggerISO=true;
				...
				} //note: hard photons imply later a DR cone rejection wrt the jets
